...
Since it was my first time, I had no clue why we needed all these parts. I compiled a shopping list, keeping some nuts and bolts as extras. For a comprehensive **list of parts** I used, refer to `parts_list.md` in this repo.
...
- Put a short circuit jumper across these. This enables the shield.
...
- Do a test run of the motors and drivers. The test code available on this repo can be used (see CNC_shield_test). I found it on this [YouTube video](https://youtu.be/zUb8tiFCwmk).
- Simply attach the motors to the shield, upload the code to the arduino, power on the DC adapter and the two motors should gently spin with a short pause right after.
...
- The cable on the servo is tiny. It must be crimped and extended.
...
2. Bolt on the stepper motor (NEMA 17) to the right side. Install the GT2 pulley on it, aligning it with the profile.
3. Bolt on the Idler to the left.
4. Attach the top x-gantry plate to the 2020 aluminium profile with M5 bolts and T-nuts. Fasten tightly towards one side of the 2020 profile. Leave enough from the edge, we will attach the 2nd stepper motor here later.
5. Insert the M5 45mm bolts in the holes of the x-axis gantry plate. Flip it over so that the bolt heads are on the table-top. Insert the spacers - two normal and two eccentric. Now insert the shim washers and then the wheels. Now insert the extra (round) spacers.
6. Once all the parts for the wheels have been inserted onto the bolts, put the wheels & gantry onto the profile. Ensure that the belt cutouts are in line with where the belt will be installed on the pulleys.
Now attach the other x-axis gantry plate, and use the hex (or nyloc) nuts to complete the x-axis gantry assembly.
...
3. Attach the NEMA with the spindle pointing towards the cieling. The idea is to later have the belt running along the front and the back of 2020 profile.
4. Attach the GT2 pulley on the NEMA and similarly attach the Idler on the other side.
...
Pen Clip:
1. First, use an M5 bolt to the attach the fixed end of the clip to the pen lifter. Use a 5mm hex nut to fasten the bolt.
2. Next, fit the 3D printer spring on the circular plastic extrusions on the two halves of the clip.
3. Insert an M5 bolt while aligning the holes of the two halves to join and complete the clip.
...
2. Connect the motor on the 2040 profile to the pins of the x-axis stepper driver on the CNC shield. Use a long cable.
3. Connect the motor on the 2020 profile to the pins beside the y-axis stepper driver.
4. Connect the servo to the pins as shown in the image.
5. Connect the CNC shield to the arduino. The cutouts of the boards should be aligned.
...
3. Install a G-code sender. I used UGS (Universal G-Code Sender):

4. Install Inkscape (to send G-Code for an image):

5. Install plugins for Inkscape:
